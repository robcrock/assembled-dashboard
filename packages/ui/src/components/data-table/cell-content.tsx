...
//
// This is the authoring grammar. A column writes ONE anatomy in its `cell`, and
// any part of it the operator controls is built with the `content` builder
...
//   )
//
// THE ANATOMY IS NEVER GIVEN THE STATE. There is no `editing` flag to branch on
// and no second renderer to keep in sync, so an author has no way to make the
// read face and the edit face disagree.
//
// PRESENCE IS THE CAPABILITY. `edits` names the row key, so the value shown and
// the value committed cannot disagree — same key, read once. There is no
// `editable: true`, no `readOnly`, and deliberately NO `reads` counterpart: a
// read-only value is better written as the primitive it already is
// (`<Duration seconds={q.longest_wait_sec} />`), which is what both dashboard
// tables do.
//
// The word `field` is banned here on purpose. In Airtable, Notion, SQL and AG
// Grid's `colDef.field`, a FIELD IS A COLUMN — it points one tier up from where
// it renders. What's inside a cell is its CONTENT.

/* ---- the contract --------------------------------------------------------- */

/**
 * Content a cell shows. `show` is the value in its DISPLAY representation — the
 * read face, and the only thing a cell without a box ever renders.
 *
 * Sets no type scale: that belongs to the td and the anatomy, never to the
 * value. A `show` that declared its own size would restyle its cell on entering
 * edit mode.
 */
 ...
 * and is read by the reducer, so a second declaration here would be drift.
 */
export interface EditCapability<V, Draft> extends EditBehavior<V, Draft> {
  /**
   * An element FACTORY the cell CALLS — never a component type it mounts. A
   * per-render arrow used as an element TYPE remounts the control on every
   * poll tick, eating the caret and any in-flight IME composition. Called, the
   * element's type is the stable inner primitive.
   */
   ...
   *
   * `show` cannot do this job: an SLA target DISPLAYS as `2m` and is TYPED as
   * `120`, so a box sized from `show` clips the live control down to its unit
   * suffix. `show` is how the value READS; `showDraft` is how the draft TYPES,
   * and the box is sized from the latter. Identical for text/number/enum — the
   * duration pair is the only one whose two representations genuinely differ.
   * The author writes NEITHER, so they cannot be made to disagree.
   */
 ...
 * capability is complete or it does not exist. Sibling optionals could express
 * combinations that are nonsense; a nested one cannot.
 */
 ...
 * change without touching anything. That holds only while there is exactly ONE
 * recipe and nothing but this module can apply it: it is package-private, and
 * applied by the only thing that mounts a control.
 *
 * The border is muted INK at low alpha, deliberately NOT `border-input`:
 * `--input`, `--border` and `--muted` all resolve to the same concrete step, so
 * an input-bordered box on a selected row (`bg-muted`) draws its border in the
 * row's own colour and vanishes — selection would erase the very affordance
 * that says "this is editable".
 *
 * `align-middle` + `-my-0.5` keep an inline-flex box from growing the line box
 * that holds it: a 24px (h-6) box inside a 20px text line would make the td
 * taller than the row rhythm, and the negative margin returns its contribution
 * to the line's own 20px.
 *
...
/**
 * Mounts the live control. MODULE-LEVEL and stable, with `control` arriving as a
 * PROP rather than as the element type: because this component's identity never
 * changes, a poll tick re-rendering the table updates the control in place
 * instead of tearing it down, and the caret survives.
 *
...
/**
 * The one gesture that changes an editable value. This reports intents; the
 * container supplies the policy (see lib/editor.ts).
 */
...
interface CellValueProps<V, Draft> {
  ...
  value: V
  address: ContentAddress
  /** May this value be changed right now. Capability without permission still reads. */
  permitted: boolean
  slot: EditSlot | null
 ...
 * figure jumping on click. The committed face stays in flow while editing as an
 * invisible, aria-hidden WIDTH STRUT, and the control lays over it — chromeless,
 * at the inherited type scale — so the box measures the same before and after
 * the click, and a poll tick can't resize it under the caret.
 */
export function CellValue<V, Draft>({
  ...
  // actually type it in (`120s`, where the read face says `2m`). A box is a
  // promise that a click edits THIS; it should show the thing it is promising.
  ...
  // An uncommittable draft (an emptied field is exactly that) has nothing to
  // measure, so the box holds at the committed width rather than collapsing to
  ...
  const editorProps: EditorProps<Draft> = {
    ...
    // must not paint a second one, and it inherits the anatomy's type scale
    // rather than setting its own.
    //
    // No positioning here — `EditorProps.className` reaches whatever paints the
    // border, never a positioning wrapper, so `absolute` would land on the
    // input while its own wrapper stayed a flex sibling of the strut. The
    // overlay below is the cell's own.
    //
    // The `md:` twin is load-bearing: the vendored Input ships `md:text-sm`, a
    // variant beats a base utility at every width this dashboard is used at,
    // and tailwind-merge won't dedupe across variant groups — so a bare
    // `text-[length:inherit]` loses and the control types bigger than it rests.
    className:
  ...
  return (
    <span
      ...
      // listener is never attached. The machine independently ignores a blur it
      // shouldn't get, so this is belt and braces, not the only guard.
      onBlur={
        content.edit.popup
          ? undefined
          : (event: React.FocusEvent<HTMLSpanElement>) => {
              if (!event.currentTarget.contains(event.relatedTarget)) {
                // `state.draft` and not a live copy: a departure is always at
                // least a render after the keystroke that changed the draft, so
                // the slot has it. Only the picker's same-tick pick is racy, and
                // that rides in on `onCommit`'s own argument.
                events.onCommit(
                  address,
                  content.edit,
                  value,
    ...
    >
      {/* The strut: the box's ONLY in-flow child, so it alone decides the width,
          and the width it decides is the draft's. */}
      ...
      {/* The overlay: the cell's own positioning wrapper, laid exactly over the
          strut — an editor styles its FIELD and keeps its own wrapper, so this
          cannot ride in on the editor's className. */}
 ...
 * floor. The word "observation" appears nowhere in the code — it is expressed by
 * ABSENCE from the union, and the compile error is the word.
 *
 * These helpers then check that the key's VALUE SHAPE matches the builder being
 * called.
 */
 ...
 * builder mints the content, addresses it, and renders it, so a call site writes
 * the value where it belongs in the anatomy and nothing else.
 *
...
> {
  ...
  /*
   * Generic over the KEY, not the value. With `<V extends string>` inferred from
   * `options`, V floats free of the key and `{ edits: "state", options:
   * QUEUE_OPTIONS }` typechecks — a picker offering values the key cannot hold.
   * Binding the options to `Row[K]` checks the key AND its value shape.
   */
  ...
  return {
    text: (cfg) =>
      render<string, string>(cfg.edits as string, {
        ...
        edit: {
          ...
          draft: (value) => value,
          // An emptied name is a draft, never a commit: a queue called "" is not
          // a thing the floor can have.
    ...
    number: (cfg) =>
      render<number, number | null>(cfg.edits as string, {
        // `tabular-nums` and nothing else. Tabular figures are a property of the
        // VALUE (a ticking number must not jitter its own width); the type SCALE
        // belongs to the anatomy that holds it.
        show: (value) => (
        ...
        edit: {
          // No `unit` here, deliberately: NumberFieldEditor renders a unit as an
          // absolutely-pinned suffix plus 32px of reserved padding — chrome
          // sized for a form field, not a 30px cell. In this box the unit is the
          // BOX's to say (it is in `showDraft`, and the strut reserves its
          // width).
          control: (props) => (
    ...
    enum: (cfg) =>
      render(cfg.edits as string, {
        ...
        // labels are welcome: pass the display primitive and the picker previews
        // exactly what the cell will render.
        show: (value: string) =>
    ...
    multiselect: (cfg) => {
      // ONE renderer, referenced twice — not two copies that happen to match. A
      // membership list reads and types identically.
      //
      // A COMMA, not a middot: the middot separates things of DIFFERENT kinds
      // ("Breached · 55s over"), while a membership list is one kind repeated —
      // and the repo already punctuates exactly this list exactly this way
      // (queue-coverage.tsx). It reads better too: a middot is aria-hidden by
      // convention, leaving a screen reader saying "Billing Tier 2".
